...
def plots(e, v, signals=(True, False), furie=True, phase=True, wavelet=False):
    ...
    options = SolveOptions(None, "", t_0, t_end, T_tran, dt, False, atol=1e-7, rtol=1e-4)
    ...
    # plot time signal:
    if signals[0]:
        axs["signal"].plot(time[:int(time_plot/dt)], data[val, :int(time_plot/dt)], lw=0.3, color='C0')
        axs["signal"].set_xlabel("t")
        axs["signal"].set_xlim(T_tran, T_tran+time_plot)
        axs["signal"].set_ylabel(f'{key}')
    ...
    if signals[1]:
        ...
        n +=1
        axs["cutted_signal"].plot(time[:int(time_plot/dt)], data[val, :int(time_plot/dt)], lw=0.3, color='C0')
        axs["cutted_signal"].set_xlabel("t")
        axs["cutted_signal"].set_ylabel(f'{key}')
    ...
    # plot phase
    if phase:
        # axs["phase"] = plt.axes(projection='3d')
        axs["phase"].set_title(f'$\t{list_title[n]}$', x=0.94, y=0.7+c)
        n +=1
        axs["phase"].plot(data[4], data[1], '.', ms=0.1, color='black')
        axs["phase"].set_xlabel("Y")
        axs["phase"].set_ylabel("W")
        # axs["phase"].set_ylabel('W')

    # plot wavelet
    if wavelet:
        Tx, Wx, ssq_freqs, _ = ssq_cwt(data[val], ssq_freqs='linear', fs=1/dt, maprange='energy', wavelet='bump', nv=128)
        ...
        axs["wavelet"].set_xlim(T_tran, T_tran+time_plot)
        axs["wavelet"].set_xlabel("t")
        axs["wavelet"].set_ylabel('v')


plots(205, 4.25, signals=(True, False), furie=True, phase=True, wavelet=True)
plt.show()